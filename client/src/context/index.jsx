...
export const StateContextProvider = ({children}) => {
  const { contract } = useContract("0xa4880f8E51490426c6B112d3f6e3117090bd6f09");
  ...
  const { mutateAsync: createCampaign } = useContractWrite(contract, "createCampaign");
	...
	const getCampaigns = async () => {
		...
		const parsedCampaigns = campaigns.map((campaign, i) => ({
		  ...
		  amountCollected: caver.utils.fromPeb(campaign.amountCollected.toString(), 'KLAY'),
	...
	const donate = async (pId, amount) => {
		const data = await contract.call('donateToCampaign', [pId], {value: caver.utils.toPeb(amount, "KLAY")});
		return data;
	}
