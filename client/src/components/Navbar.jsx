...
const Navbar = ({ searchFilter, metamaskInstalled }) => {
  ...
  return (
    <div className="flex md:flex-row flex-col-reverse justify-between mb-[35px] gap-6">
      <div
        className={`lg:flex-1 flex flex-row max-w-[458px] py-2 pl-4 pr-2 h-[52px] bg-[${palette.background.dark}] rounded-[100px]`}
      >
        <input
        ...
        <div
          className={`w-[72px] h-full rounded-[20px] bg-[${palette.primaryMain}] flex justify-center items-center cursor-pointer`}
        >
          <img
            src={search}
            alt="search"
      ...
      <div className="sm:hidden flex justify-between items-center relative">
        <div
          className={`w-[40px] h-[40px] rounded-[10px] bg-[${palette.background.dark}] flex justify-center items-center cursor-pointer`}
        >
          <img
            src={logo}
            alt="user"
            ...
            <ul className="mb-4">
              {navlinks.map((link) => (
                <li
                  key={link.name}
                  className={`flex p-4 ${
                    isActive === link.name && `bg-[${palette.background.dark}]`
                  }`}
                  onClick={() => {
